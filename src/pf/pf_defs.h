...
class PageId{
  public:
    ...
    friend bool operator==(const PageId &x, const PageId &y) { return x.fd == y.fd && x.page_no == y.page_no; }   //重载=运算符 使其可以比较结构体
    friend bool operator!=(const PageId &x, const PageId &y) { return !(x == y); }

    friend std::ostream &operator<<(std::ostream &os, const PageId &self) {                                       //重载输出运算符
